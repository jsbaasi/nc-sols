class MedianFinder:
    def __init__(self):
        ...
        self.pop = lambda h: heapq.heappop(self.min_heap) if h==self.min_heap else heapq.heappop_max(self.max_heap)
        self.push = lambda h,val: heapq.heappush(self.min_heap, val) if h==self.min_heap else heapq.heappush_max(self.max_heap, val)
    ...
    def addNum(self, num: int) -> None:
        if not self.min_heap or num<self.min_heap[0]: heapq.heappush_max(self.max_heap, num)
